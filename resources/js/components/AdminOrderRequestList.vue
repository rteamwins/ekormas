                ...
                <tr v-for="(order, i) in orders" :key="`order_${i}`">
                  ...
                  <td>
                    ...
                    <span
                      ...
                      style="cursor:pointer;"
                      @click="complete_order(order.id)"
                      class="uk-icon-button green-text text-lighten-2"
                      uk-icon="check"
                    ></span>
                  </td>
                </tr>
...
export default {
  ...
  methods: {
    ...
    load_data(page = 1) {
      ...
      axios
        .get(`${this.base_url}/api/admin/order/list?page=${page}`)
        .then(res => {
    ...
    ship_order(order_id) {
      let current_page = this.order_pagination_data.current_page;
      this.$swal
        .fire({
          title: "Shipped?",
          text: "Mark This Order As Shipped!",
          icon: "info",
        ...
        .then(result => {
          if (result.value) {
            axios
              .get(
                `${window.location.origin}/api/admin/order/mark-as/shipped/${order_id}`
              )
              .then(res => {
    ...
    view_buyer(buyer) {
      this.$swal.fire({
        title: buyer.name,
        html: `<b>Phone:</b> ${buyer.phone}<br><b>Username:</b> ${buyer.username}<br><b>Email:</b> ${buyer.email}`,
      ...
      this.$swal
        ...
        .then(result => {
          if (result.value) {
            axios
              .get(
                `${window.location.origin}/api/admin/order/mark-as/completed/${order_id}`
              )
              .then(res => {
                this.load_data(current_page);
                this.$swal.fire({
